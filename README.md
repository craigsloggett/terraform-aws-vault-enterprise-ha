...
| <a name="input_hcp_terraform_hostname"></a> [hcp\_terraform\_hostname](#input\_hcp\_terraform\_hostname) | The HCP Terraform hostname. Override for Terraform Enterprise installations. | `string` | `"app.terraform.io"` | no |
| <a name="input_hcp_terraform_org_name"></a> [hcp\_terraform\_org\_name](#input\_hcp\_terraform\_org\_name) | The HCP Terraform organization name. Used to scope the JWT auth role for the Vault admin workspace. | `string` | n/a | yes |
| <a name="input_hcp_terraform_workspace_id"></a> [hcp\_terraform\_workspace\_id](#input\_hcp\_terraform\_workspace\_id) | The HCP Terraform workspace ID that will manage Vault configuration post-bootstrap. Scoped tightly to encourage all Vault config changes to flow through this workspace. | `string` | n/a | yes |
