...
| <a name="input_enable_vault_agent"></a> [enable\_vault\_agent](#input\_enable\_vault\_agent) | Enable the Vault Agent service. When true, agent starts after the initial manual cert issuance and takes ownership of subsequent TLS certificate rotations. When false (default), agent files exist on disk but the service is not started. | `bool` | `false` | no |
...
| <a name="input_vault_audit_disk"></a> [vault\_audit\_disk](#input\_vault\_audit\_disk) | EBS configuration for the Vault audit log volume (/dev/xvdg). | <pre>object({<br/>    volume_type = string<br/>    volume_size = number<br/>    encrypted   = bool<br/>  })</pre> | <pre>{<br/>  "encrypted": true,<br/>  "volume_size": 50,<br/>  "volume_type": "gp3"<br/>}</pre> | no |
